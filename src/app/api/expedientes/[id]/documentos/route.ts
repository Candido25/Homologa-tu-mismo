...
export async function GET(_request: Request, context: RouteContext) {
  ...
  try {
    ...
    return NextResponse.json(
      { error: "No pudimos consultar los documentos del expediente." },
      { status: 500 },
    );
  }
}
...
export async function POST(request: Request, context: RouteContext) {
  ...
  if (Number.isFinite(contentLength) && contentLength > maxDocumentSize + 1024 * 1024) {
  ...
  try {
    ...
    return NextResponse.json({ error: "La carga enviada no es válida." }, { status: 400 });
  }
