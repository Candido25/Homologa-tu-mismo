...
export async function POST(request: Request, context: RouteContext) {
  ...
  if (!/^[a-z][a-z0-9_]{1,63}$/.test(documentTypeCode)) {
    return NextResponse.json(
      { error: "El tipo documental no es válido." },
      { status: 422 },
    );
  }
  ...
  try {
    ...
    console.error("document_upload_api_failed", {
      caseId,
...
export async function GET(_request: Request, context: RouteContext) {
  ...
  try {
    ...
    console.error("document_list_api_failed", {
      caseId,
      message: error instanceof Error ? error.message : "unknown",
    });
    return NextResponse.json(
      { error: "No pudimos recuperar la lista de documentos." },
      { status: 500 },
    );
  }
}
