...
El propietario dispone de una suscripción activa Azure for Students. Se decide aprovechar esta etapa temprana para construir la plataforma definitiva en Azure, manteniendo tecnologías portables y controles de costo.

## Decisión

Azure será la plataforma principal de desarrollo y producción. Los componentes se separan por responsabilidad:
...
- **Costos:** Azure Cost Management con presupuesto y alertas antes de producción.
...
2. **Desarrollo y producción separados.** Se usarán grupos de recursos distintos: `rg-homologa-dev` y `rg-homologa-prod`.
3. **Sin secretos en GitHub ni en código.** App Service usará identidad administrada y Key Vault.
4. **Privacidad por defecto.** Los contenedores no permiten acceso público y la aplicación autoriza cada operación.
5. **Portabilidad.** PostgreSQL, Next.js y formatos estándar reducen la dependencia irreversible del proveedor.
6. **Infraestructura reproducible.** Ningún recurso esencial dependerá únicamente de configuraciones manuales del portal.
7. **Costo controlado.** Alta disponibilidad, redes privadas y escalado superior se activarán cuando el riesgo y los ingresos lo justifiquen.
...
- Producción no utilizará la suscripción estudiantil como única garantía de continuidad: antes del lanzamiento se documentará la transición a pago por consumo.
...
- Una sola plataforma operativa y de facturación.
...
- Azure Database for PostgreSQL puede consumir una parte importante del crédito estudiantil.
