...
export async function GET(request: Request, context: RouteContext) {
  ...
  try {
    ...
    return NextResponse.json({ documents: documents || [] }, { status: 200, headers: { "Cache-Control": "private, no-store" } });
  } catch (error) {
...
export async function POST(request: Request, context: RouteContext) {
  ...
  if (file.size > maxDocumentSize) {
    return NextResponse.json({ error: "El documento supera el límite." }, { status: 413 });
  }

  const content = new Uint8Array(await file.arrayBuffer());
  if (!hasValidDocumentSignature(content, mimeType)) {
    return NextResponse.json({ error: "Contenido no válido." }, { status: 422 });
  }
  ...
  try {
    ...
    if (!result.ok) {
      const isNotFound = result.reason === "case_not_found";
      return NextResponse.json({ error: "No válido." }, { status: isNotFound ? 404 : 422 });
    }
